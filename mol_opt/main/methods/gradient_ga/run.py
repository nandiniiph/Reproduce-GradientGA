...
from main.methods.gradient_ga.gradient_ga import Gradient_GA_Optimizer as Optimizer
...
def mol_to_dgl(mol):
    ...
    for u in range(num_atoms):
        ...
        h_u = [int(atom.GetSymbol() == x) for x in ATOM_TYPES]
        h_u += [atom.GetAtomicNum(), int(atom.GetFormalCharge()), int(atom.GetIsAromatic())]
        h_u += [int(atom.GetHybridization() == x) for x in HYBRID_TYPES]
        h_u.append(atom.GetTotalNumHs())
        atom_feats_list.append(h_u)
        atom_feats_dict['node_type'].append(atom.GetAtomicNum())
        atom_feats_dict['node_charge'].append(atom.GetFormalCharge())
    ...
    for bond in mol.GetBonds():
        ...
        edges += [(u, v), (v, u)]
        bf = [float(bond.GetBondType() == x) for x in BOND_TYPES]
        bond_feats += [bf, bf]

    if edges:
        src, dst = zip(*edges)
        g.add_edges(src, dst)
        g.edata['e_feat'] = torch.FloatTensor(bond_feats)

    return g

def make_mating_pool(population_mol, population_scores, offspring_size):
    scores = [abs(s) + MINIMUM for s in population_scores]
    probs = [s / sum(scores) for s in scores]
    return np.random.choice(population_mol, p=probs, size=offspring_size, replace=True)

def print_results(t, a, b, x):
    directory = "results/gradient_ga"
...
class GradientGA_Run:
    def __init__(self, args):
        self.args = args

    def run(self):
        # Import optimizer di sini supaya circular import hilang
        from main.methods.gradient_ga.gradient_ga import Gradient_GA_Optimizer
        optimizer = Gradient_GA_Optimizer(self.args)
...
def _result_dir(method, oracle):
    return os.path.join("results", method, oracle)
...
class Gradient_GA_Optimizer(BaseOptimizer):
    ...
    def __init__(self, args):
        super().__init__(args)
        self.runner = GradientGA_Run(args)
    ...
    def _optimize(self, oracle, config):
        ...
        population_smiles = np.random.choice(
            self.all_smiles, config["population_size"]
        )
        ...
        while not self.finish:
            mating_pool = make_mating_pool(
                population_mol,
                self.oracle([Chem.MolToSmiles(m) for m in population_mol]),
                config["population_size"]
            )

            parent_a, parent_b = random.sample(list(mating_pool), 2)
            children = co.get_co_space(parent_a, parent_b)

            if not children:
                continue

            offspring = random.sample(
                children, min(len(children), config["offspring_size"])
            )
            offspring = [mu.mutate(m, config["mutation_rate"]) for m in offspring]
            offspring = [m for m in offspring if m is not None]

            population_mol.extend(offspring)
            population_mol = self.sanitize(population_mol)

            self.log_intermediate(finish=False)

        return self.log_intermediate(finish=True)
